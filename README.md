...
现代异步、工程洁癖、性能和可维护性，以及 FastAPI、aiohttp、APScheduler、Vue、ECharts、vLLM 等技术选择，都属于 `super_bill` 默认工程偏好，不是绝对要求；不得覆盖用户要求、现有项目约束、当前官方兼容性和安全要求。
...
- workflow 使用 job timeout、SSH BatchMode/连接超时/keepalive 和 Base64 数据传输，避免远端 shell 引号注入。
- 回滚会按旧锁文件重新执行 `uv sync --frozen` 或 `npm ci`，并分项报告恢复状态；失败明确标记 `CRITICAL`。
...
`validate-skills.py` 验证五 Skill 自包含结构和确认需求矩阵；`test-behavior-contract.py` 验证六个典型场景的规则覆盖；`test-regressions.py` 动态验证同秒备份、用户自定义入口保留、Node 回滚依赖恢复、脏工作树门禁、回滚告警和 Base64 传输。它们不冒充独立模型行为测试，发布前仍需测试 Agent 做真实前向检查。
...
卸载时不会直接删除安装产物，而是移动到带时间戳、进程号和冲突序号的唯一备份，并从 `AGENTS.md`/`CLAUDE.md` 移除 Doctor Bill 标记区块。
