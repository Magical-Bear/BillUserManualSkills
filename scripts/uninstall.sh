...
Uninstall moves skill directories to timestamped backups and removes marked Doctor Bill blocks from adapter files.
USAGE
}
...
remove_marked_block() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    log "[dry-run] remove DOCTOR-BILL block from $target"
    return 0
  fi
  python3 - "$target" <<'PY'
...
uninstall_skill_set() {
  ...
  for skill in "${SKILLS[@]}"; do
    move_to_backup "$skills_dir/$skill"
  done
}

uninstall_codex() {
  uninstall_skill_set "$CODEX_HOME/skills"
  remove_marked_block "$CODEX_HOME/AGENTS.md"
}

uninstall_claude() {
  uninstall_skill_set "$CLAUDE_HOME/skills"
  remove_marked_block "$CLAUDE_HOME/CLAUDE.md"
}
...
uninstall_cursor() {
  if [[ -z "$CURSOR_RULES_DIR" ]]; then
    if [[ -d "$HOME/.cursor/rules" ]]; then
      ...
      log "Cursor rules dir not known. Pass --cursor-rules-dir. Skipping Cursor."
      return 0
    fi
  fi
  for file in doctor-bill.mdc; do
    move_to_backup "$CURSOR_RULES_DIR/$file"
  done
}
